...
Vue.filter('sbercoin', (satoshis, precision = null) => {
  if (precision == null) {
    let s = satoshis.toString().padStart(8, '0')
    return addAmountDelimiters((s.slice(0, -7) + '.' + s.slice(-7)).replace(/\.?0*$/g, ''))
  } else {
    return addAmountDelimiters((satoshis / 1e7).toFixed(precision))
  }
})
