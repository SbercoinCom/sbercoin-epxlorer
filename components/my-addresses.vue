<template>
  <div v-show="addresses.length">
    <button class="button is-sbercoin" @click="show = !show">
    ...
    <table v-show="show">
      <tbody>
        <tr v-for="{address, balance} in list">
          ...
          <td class="monospace has-text-right">{{ balance | sbercoin(7) }}</td>
          <td>
      ...
      <tfoot v-if="addresses.length > 1">
        <tr>
          ...
          <td class="monospace has-text-right">{{ totalBalance | sbercoin(7) }}</td>
          <td></td>
        </tr>
      </tfoot>
    </table>
